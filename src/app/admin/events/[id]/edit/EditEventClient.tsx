...
export default function EditEventClient({ event }: { event: any }) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        // Pass the event ID to the update action
            ...
            <div className="flex items-center gap-6">
                ...
                <div>
                    <h1 className="text-4xl font-black italic uppercase tracking-tighter text-primary">
                        Modify <span className="text-accent underline">Event</span>
                    </h1>
            ...
            <form onSubmit={handleSubmit} className="bg-white p-12 rounded-[3.5rem] shadow-sm border border-slate-100 space-y-10">
                <div className="grid md:grid-cols-2 gap-10">
                    <div className="space-y-4">
                        ...
                        <input name="title" defaultValue={event.title} required className="w-full p-6 bg-slate-50 rounded-3xl outline-none focus:ring-4 ring-accent/30 transition-all font-black text-primary" placeholder="e.g. Annual Sports Meet" />
                    </div>
                    <div className="space-y-4">
                        <label className="text-xs font-black uppercase text-slate-400 tracking-widest ml-2">Image URL</label>
                        <input name="image" defaultValue={event.image} className="w-full p-6 bg-slate-50 rounded-3xl outline-none focus:ring-4 ring-accent/30 transition-all font-black text-primary" placeholder="https://unsplash.com/..." />
                    </div>
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-10">
                    <div className="space-y-4">
                        ...
                        <input name="date" type="date" defaultValue={event.date} required className="w-full p-6 bg-slate-50 rounded-3xl outline-none focus:ring-4 ring-accent/30 transition-all font-black text-primary" />
                    </div>
                    <div className="space-y-4">
                        <label className="text-xs font-black uppercase text-slate-400 tracking-widest ml-2">Time</label>
                        <input name="time" type="time" defaultValue={event.time} required className="w-full p-6 bg-slate-50 rounded-3xl outline-none focus:ring-4 ring-accent/30 transition-all font-black text-primary" />
                    </div>
                    <div className="space-y-4">
                        <label className="text-xs font-black uppercase text-slate-400 tracking-widest ml-2">Status</label>
                        <select name="isPast" defaultValue={event.isPast ? "true" : "false"} className="w-full p-6 bg-slate-50 rounded-3xl outline-none focus:ring-4 ring-accent/30 transition-all font-black text-primary appearance-none">
                ...
                <div className="space-y-4">
                    ...
                    <input name="venue" defaultValue={event.venue} required className="w-full p-6 bg-slate-50 rounded-3xl outline-none focus:ring-4 ring-accent/30 transition-all font-black text-primary" placeholder="Kottoor Backwaters" />
                </div>

                <div className="space-y-4">
                    <label className="text-xs font-black uppercase text-slate-400 tracking-widest ml-2">Intelligence (Description)</label>
                    <textarea name="description" defaultValue={event.description} required className="w-full p-6 bg-slate-50 rounded-3xl h-48 outline-none focus:ring-4 ring-accent/30 transition-all font-black text-primary resize-none" placeholder="Describe the significance of this event..."></textarea>
                </div>
                ...
                >
                    <Save size={24} />
                    {isLoading ? "Synchronizing..." : "Update Event"}
                </button>
            </form>
        </div>
    );
